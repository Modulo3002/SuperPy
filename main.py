...
report_parser.add_argument("report_time", choices=["today", "yesterday", "date"], help="Choose a revenue report time. Choices are 'today'or 'yesterday'")
report_parser.add_argument("custom_date", nargs="?", help="Specify a custom date in format YYYY-MM-DD (required if 'report_time' is 'date').")
...
report_parser.add_argument("quantity", type=int, help="How many of the product to sell")
...
def main():
    ...
    if args.command == "report_revenue":
        ...
        if args.report_time == "date":
            target_date = dt.datetime.strptime(args.custom_date, "%Y-%m-%d").date()
            report_for_date(target_date)
